...
namespace Application
{
    ...
    void Semaphore::notify(int threadId)
    { // threadId only used during logging/debug
    ...
    void Semaphore::wait(int threadId)
    { // threadId only used during logging/debug
        std::unique_lock<std::mutex> lock(mtx);

        while (count == 0)
        { // if count is zero, critial region can't be accessed
